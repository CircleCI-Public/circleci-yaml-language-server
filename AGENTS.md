...
stdlib's map with `cmp.DeepEqual` where possible.

Higher level happy path tests should be added to the acceptance tests.

Lower level sad path / error cases should be tested in the api package tests.
...
```go
t.Run("Happy path", func(t *testing.T) {
    t.Run("do some setup", (t *testing.T) {
        ...
    })
    t.Run("make an HTTP request", (t *testing.T) {
        ...
    })
    t.Run("check a side effect happened", (t *testing.T) {
        ...
    })
})
```
