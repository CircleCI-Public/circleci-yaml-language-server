...
class RpcClient<O extends RequestDefinitions> extends EventTarget {
    ...
    async request<T extends keyof O>(
        ...
        return this.requestRaw(method as string, params) as unknown as
            | ProtocolReturns
            | undefined;
    }

    async requestRaw(method: string, params: RequestResult) {
        ...
        this.dispatchEvent(
            new CustomEvent(EventType.requestWillBeSent, {
        ...
        await new Promise((resolve) => {
            socket.write(message, (err) => {
                ...
                this.dispatchEvent(
                    new CustomEvent(EventType.requestSent, {
                        detail: { request },
                    }),
                );
                resolve(undefined);
    ...
    async connect(timeout: number): Promise<boolean> {
        ...
        this.#connectingPromise = connectToServer(
            this.port,
            this.host,
            timeout,
        ).then((socket) => {
    ...
    async disconnect(graceTime = 10): Promise<boolean> {
        ...
        this.#endingPromise = Promise.race([closePromise, timeoutPromise]);

        return this.#endingPromise;
    }

    #dispatchErrorEvent(context: string, error: unknown) {
        const event = new CustomEvent(EventType.error, {
            detail: {
                context,
                error,
            },
     ...
     */
    #onData(data: Buffer) {
        const payloads = this.#messageParser.parseMessage(data);

        for (const payload of payloads) {
            if (!payload) {
                this.#dispatchErrorEvent(
                    'Received message with no body',
                    data.toString(),
                );
                return;
            }

            const event = new CustomEvent(EventType.dataReceived, {
                detail: { data: payload },
            });
            ...
            if (promiseHandler === undefined) {
                this.#dispatchErrorEvent(`Unknown request id: ${id}`, {
                    payload,
                });
                return;
            }
...
function buildMessage(payload: RequestPayload): string {
    ...
    const message = [contentType, contentLength, '', `${content}`].join('\r\n');
