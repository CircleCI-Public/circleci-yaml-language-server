...
func (val Validate) doesJobInvocationExist(jobInvocations []ast.JobInvocation, requireName string) bool {
	for _, jobInvocations := range jobInvocations {
		if jobInvocations.JobName == requireName || jobInvocations.StepName == requireName {
			return true
		}
	}
	return false
...
// Validates and adds diagnostics for job invocations within a workflow.
