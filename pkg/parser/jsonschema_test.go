	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				// Use relative path to schema.json from pkg/parser
				...
				// TODO: These tests are currently failing because gojsonschema may not
				// fully support JSON Schema draft-06 if/then/else conditionals.
				// Uncomment the assertions below once the schema validation is fixed.
