...
func (ch *CompletionHandler) addCheckoutMethodCompletion() {
	for _, method := range ast2.CheckoutMethods {
		ch.addCompletionItem(method)
	}
}

func (ch *CompletionHandler) addCompleteEnvVariables(contexts []string, parameters map[string]ast2.Parameter, environmentField map[string]string) {
