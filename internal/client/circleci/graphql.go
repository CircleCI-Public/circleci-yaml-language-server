...
// NewGraphQLClient returns a reference to a Client.
func NewGraphQLClient(host, endpoint, token string, debug bool) *GraphQLClient {
	...
	return &GraphQLClient{
		...
		httpClient: client.New(httpcl.Config{Transport: transport}),
		Endpoint:   endpoint,
		Host:       host,
		Token:      token,
		Debug:      debug,
...
// NewGraphQLRequest returns a new GraphQL request.
...
func (request *GraphQLRequest) SetToken(token string) {
	request.Header.Set("Authorization", token)
}

// SetUserId sets the Authorization header for the request with the given user id.
func (request *GraphQLRequest) SetUserId(userId string) {
	request.Header.Set("user_id", userId)
}

// Var sets a variable.
func (request *GraphQLRequest) Var(key string, value interface{}) {
	request.Variables[key] = value
}

// Encode will return a buffer of the JSON encoded request body
func (request *GraphQLRequest) Encode() (bytes.Buffer, error) {
	var body bytes.Buffer
	err := json.NewEncoder(&body).Encode(request)
	return body, err
}

// GraphQLResponse wraps the result from our GraphQL server response including out-of-band errors and the data itself.
type GraphQLResponse struct {
	Data   interface{}
	Errors GraphQLErrors
}

// GraphQLErrors represents a slice of errors returned by the GraphQL server out-of-band from the actual data.
type GraphQLErrors []GraphQLError

// GraphQLError represents the key-value pair of data returned by the GraphQL server to represent errors.
...
func (cl *GraphQLClient) RunWithContext(ctx context.Context, request *GraphQLRequest, resp interface{}) error {
	...
	wrappedResponse := &GraphQLResponse{
		Data: resp,
	}
