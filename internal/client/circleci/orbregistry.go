...
func NewOrbRegistry(hostUrl, token, userId string, debug bool) OrbRegistry {
	return &fallbackOrbRegistry{
		...
		gql:  graphqlOrbRegistry{client: NewGraphQLClient(hostUrl, "graphql-unstable", token, debug), userId: userId},
	}
}
...
func (registry graphqlOrbRegistry) request(query string) *GraphQLRequest {
	request := NewGraphQLRequest(query)
