...
func TestOrbPackageName(t *testing.T) {
	for _, testCase := range []struct {
		...
		got := circleci.OrbPackageName(testCase.ref)
...
func TestSortOrbVersionsDesc(t *testing.T) {
	t.Run("orders releases newest first", func(t *testing.T) {
		versions := []circleci.OrbPackageVersion{
			{Version: "1.7.1"},
			{Version: "4.0.0"},
			{Version: "0.1.0"},
			{Version: "1.12.0"},
		...
		circleci.SortOrbVersionsDesc(versions)
...
func versionStrings(versions []circleci.OrbPackageVersion) []string {
