...
func (doc *YamlDocument) parseJobInvocations(jobsInvocationsNode *sitter.Node) []ast2.JobInvocation {
	blockSequenceNode := GetChildSequence(jobsInvocationsNode)
	jobInvocations := []ast2.JobInvocation{}
...
func (doc *YamlDocument) parseSingleJobInvocation(jobInvocationNode *sitter.Node) ast2.JobInvocation {
	res := ast2.JobInvocation{MatrixParams: make(map[string][]ast2.ParameterValue), Parameters: make(map[string]ast2.ParameterValue)}
	...
	res.Parameters = make(map[string]ast2.ParameterValue)
	res.HasMatrix = false
...
func (doc *YamlDocument) parseContext(node *sitter.Node) []ast2.TextAndRange {
	if node.ChildCount() != 1 {
		return []ast2.TextAndRange{}
	}

	if node.Kind() == "flow_node" && node.ChildCount() == 1 && node.Child(0).Kind() == "plain_scalar" {
		return []ast2.TextAndRange{doc.GetNodeTextWithRange(node)}
	}

	return doc.getNodeTextArrayWithRange(node)
}

func (doc *YamlDocument) parseSingleJobRequires(requiresNode *sitter.Node) []ast2.Require {
	blockSequenceNode := GetChildSequence(requiresNode)
	res := make([]ast2.Require, 0, requiresNode.ChildCount())
	...
	iterateOnBlockSequence(blockSequenceNode, func(requiresItemNode *sitter.Node) {
		getRequire := func(node *sitter.Node) ast2.Require {
