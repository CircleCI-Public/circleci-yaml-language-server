...
func TestYamlDocument_parseJobGroups(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields:    fields{Content: []byte(singleGroup), JobGroups: make(map[string]ast2.JobGroup)},
		...
		},
		{
			...
			fields:    fields{Content: []byte(multipleGroups), JobGroups: make(map[string]ast2.JobGroup)},
		...
		},
		{
			...
			fields:    fields{Content: []byte(emptyGroup), JobGroups: make(map[string]ast2.JobGroup)},
		...
		},
		{
			...
			fields:    fields{Content: []byte(groupNoJobsKey), JobGroups: make(map[string]ast2.JobGroup)},
