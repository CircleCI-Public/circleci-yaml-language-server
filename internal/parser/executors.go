...
func (doc *YamlDocument) parseSingleExecutor(executorNode *sitter.Node) {
	...
	if _, ok := doc.Executors[executorName]; !ok {
		baseExecutor := ast2.BaseExecutor{
			UserParameters: make(map[string]ast2.Parameter),
		}
...
func (doc *YamlDocument) parseBaseExecutor(base *ast2.BaseExecutor, nameNode *sitter.Node, blockMappingNode *sitter.Node, fn func(node *sitter.Node), nameStep string) {
	doc.iterateOnBlockMapping(blockMappingNode, func(child *sitter.Node) {
...
func (doc *YamlDocument) parseSingleExecutorMachine(nameNode *sitter.Node, valueNode *sitter.Node) ast2.MachineExecutor {
	// valueNode is a block_mapping
	res := ast2.MachineExecutor{
		IsDeprecated: false,
	}
...
func (doc *YamlDocument) parseSingleExecutorMacOS(nameNode *sitter.Node, valueNode *sitter.Node) ast2.MacOSExecutor {
	// valueNode is a block_mapping
	res := ast2.MacOSExecutor{}
...
func (doc *YamlDocument) parseSingleExecutorDocker(nameNode *sitter.Node, valueNode *sitter.Node) ast2.DockerExecutor {
	// valueNode is a block_mapping
	res := ast2.DockerExecutor{
		Image: make([]ast2.DockerImage, 0),
	}
...
func (doc *YamlDocument) parseDockerImage(imageNode *sitter.Node) ast2.DockerImage {
	// imageNode is a block_sequence_item
	dockerImg := ast2.DockerImage{}
	...
	doc.iterateOnBlockMapping(blockMappingNode, func(child *sitter.Node) {
		...
		case "image":
			...
			dockerImg.ImageValueRange.Start.Character += uint32(len(aliasRemover.FindString(doc.GetRawNodeText(valueNode))))
		case "name":
		...
		case "auth":
			...
			dockerImg.Auth = ast2.DockerImageAuth{
				Username: dict["username"],
				Password: dict["password"],
			}
		case "aws_auth":
			dict := doc.parseDictionary(GetChildOfType(valueNode, "block_mapping"))
			dockerImg.AwsAuth = ast2.DockerImageAWSAuth{
...
func (doc *YamlDocument) parseExecutorRef(valueNode *sitter.Node, child *sitter.Node) (string, protocol.Range, map[string]ast2.ParameterValue) {
	executorParameters := map[string]ast2.ParameterValue{}
	if valueNode == nil {
