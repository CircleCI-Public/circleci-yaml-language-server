...
func TestYamlDocument_parseParameters(t *testing.T) {
	tests := []struct {
		name    string
		args    parameterArgs
		wantRes map[string]ast2.Parameter
	}{
		{
    ...
    steps: { type: steps }`,
			...
			wantRes: map[string]ast2.Parameter{
				...
				"executor": ast2.ExecutorParameter{
					BaseParameter: ast2.BaseParameter{
						Name: "executor",
						NameRange: protocol.Range{
							Start: protocol.Position{
				...
				"envVar": ast2.EnvVariableParameter{
					Default: "HOME",
					BaseParameter: ast2.BaseParameter{
						Name: "envVar",
						NameRange: protocol.Range{
